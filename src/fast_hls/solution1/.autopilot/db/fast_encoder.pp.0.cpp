...
# 2 "fast_src/fast_encoder.cpp" 2
# 17 "fast_src/fast_encoder.cpp"
static const mantissa_t POW10_DEC8 = 100000000;
...
#pragma HLS ARRAY_PARTITION variable=encoded_message complete dim=1
 ...
 encoded_message[0] = 0xFC;
    ...
    VITIS_LOOP_46_1: for (unsigned i = 0; i < max_bytes_need_for_uint32 - 1; i++)
    {
    ...
    VITIS_LOOP_72_2: for (int i = 64/8 - 1; i >= 0; i--)
    {
        first_packet = (first_packet << 8)
                | encoded_message[i];
        second_packet = (second_packet << 8)
                                             ...
                                             uint8 encoded_message[16])
{
    ...
    encoded_message[exponent_offset] = 0xF8;



    mantissa_t mantissa = decoded_fix16 * POW10_DEC8;
    encode_signed_int(encoded_message, mantissa_offset, mantissa);
}

void Fast_Encoder::encode_signed_int(uint8 encoded_message[16],
                                     unsigned & mantissa_offset,
                                     mantissa_t mantissa)
{
    VITIS_LOOP_100_1: for (unsigned i = 0; i < 35/7 - 1; i++)
    {
                                           ...
                                           uint8 encoded_message[16])
{
    ...
    unsigned max_bytes_need_for_uint32 = 5;
    VITIS_LOOP_118_1: for (unsigned i = 0; i < max_bytes_need_for_uint32 - 1; i++)
    {
        uint8 curr_byte = (0x00
                | (decoded_uint32
                        >> 7
