...
typedef struct {
    ...
    XOrder_book_uio_map maps[ MAX_UIO_MAPS ];
} XOrder_book_uio_info;

/***************** Variable Definitions **************************************/
static XOrder_book_uio_info uio_info;
...
static int uio_info_read_name(XOrder_book_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XOrder_book_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XOrder_book_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XOrder_book_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XOrder_book_Initialize(XOrder_book *InstancePtr, const char* InstanceName) {
	XOrder_book_uio_info *InfoPtr = &uio_info;
